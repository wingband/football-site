    ...
    <div className="space-y-4">
      {articles.slice(0, 3).map((item, i) => {
        ...
        return (
          <a key={i} {...linkProps}>
            {item.image_url && (
              <img src={item.image_url} alt="" className="w-20 h-14 object-cover shrink-0" />
            )}
            <div className="min-w-0">
              <p className="text-sm text-floodlight/90 leading-snug line-clamp-2">{item.title}</p>
              <p className="text-xs text-floodlight/40 mt-1">
                {item.source_name} - {new Date(item.pubDate).toLocaleDateString("ko-KR")}
              </p>
            </div>
          </a>
        )
